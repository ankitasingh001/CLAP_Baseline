'''
Please install wav2vec-wrapper before use from https://github.com/Edresson/Wav2Vec-Wrapper
Modified script taken from https://github.com/Edresson/Wav2Vec-Wrapper 
Language model and lexicon files should be specified in config_example_external_LM_rescore.json
Modify the model,processor, test file path and audio file path
'''
import re
import math
import torch
...
def load_audio(batch):
    ...
    batch["speech"] = speech_array[0].numpy()#speech_array.squeeze().numpy()
...
def remove_(pred_str):
    pred_str= re.sub('<s>', '',pred_str) 
    pred_str = re.sub('N','श',pred_str)
    pred_str= re.sub('[A-Z\[\]]', '',pred_str) 
...
def test(model, test_dataset, processor, kenlm, calcule_wer=True, return_predictions=False):
        ...
        for batch in tqdm(test_dataset):
            ...
            if return_predictions:
                ...
                for i in range(len(pred_string)):
                    pred_string[i] = remove_(pred_string[i])

                for i in range(len(audios_path)):
                    print("started")
                    output_wav_path = audios_path[i]
                    #start_with_ar = dataset.filter(lambda example: example['path'].startswith(output_wav_path))
                    if dataset_base_path:
    ...
    if calcule_wer: 
        ...
        print("\n\n --> TEST PERFORMANCE\n")
        #print("     | > :   WER    ({:.5f})\n".format(avg_wer))
        print("WER: {:2f}".format(100 * wer_pred.compute(predictions=predictions[1], references=predictions[2])))
        print("CER: {:2f}".format(100 * cer_pred.compute(predictions=predictions[1], references=predictions[2])))
...
if __name__ == '__main__':
    ...
    model = Wav2Vec2ForCTC.from_pretrained("/ankita/XLSR/CLAP/final/wav2vec2-wav2vec2-large-clsril-temp")
    ...
    #Enter saved processor name
    processor = Wav2Vec2Processor.from_pretrained("/ankita/XLSR/CLAP/wav2vec2-large-clsril-hi-10")
    ...
    if not args.audio_path:
        ...
        # Enter path to test data and test csv
        path_to_dir_marathi_test = '/ankita/Data/Hindi_Data/Hindi_pratham/' #remove denoised for original verison
        line_df_test_mar = pd.read_csv("/ankita/Data/Hindi_eval_pratham.csv")
        #line_df_test_mar = line_df_test_mar.head(10)
        paths_test = []
        texts_test = []
        fils = os.listdir(path_to_dir_marathi_test)
        for fil in fils:
            if '.wav' in fil:
                try:
                    texts_test.append(line_df_test_mar[line_df_test_mar["audioFileName"]==fil]["text"].values[0])
                    paths_test.append(os.path.join(path_to_dir_marathi_test,fil))
                    #paths_test.append(fil)
                except:
                    continue
        from datasets import Dataset
        dataset = Dataset.from_dict({"path":paths_test,"text":texts_test})
